...
all_penda_res <- penda.res.sort(all_penda_res, keep.sample = c(-1,-2), gene.domain = row.names(penda_res) )
...
dereg_matrix <- get.penda.final.matrix(all_penda_res, sort = F, gene.domain = row.names(penda_res) )
```
