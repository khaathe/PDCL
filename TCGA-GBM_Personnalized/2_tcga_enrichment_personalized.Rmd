...
all_res[["G:Profiler"]] <- xfun::cache_rds({
  run.multi.gost(all_gene_list, gprofiler_gmt_list, all_tcga_genes)
}, file = "gprofiler_result", dir = result_dir, rerun = rerun_rds_cached)
```

```{r convert-to-enrichment} 
all_enrichment <- list()
all_enrichment[["G:Profiler"]] <- map_depth(all_res[["G:Profiler"]], .depth = 1, .f = convert.multi.gost.to.enrichment, colnames(penda_res))
```
