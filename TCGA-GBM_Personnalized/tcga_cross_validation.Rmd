...
source("~/PhD/PDCL/R/process-result.R")
source("~/PhD/PDCL/R/quickgo.R")
source("~/PhD/PDCL/R/kegg.R")
source("~/PhD/PDCL/R/reactome.R")
...
```{r load-controls}
control_file <- "~/PhD/PDCL/Data/PDCL/astrocyte_GSE109001_counts_unique_genes_samples_filtered.txt"
control <- read.table(
  control_file, 
  header = T, 
  sep = "\t", 
...
pdcl_count_table <- read.table(
  "~/PhD/PDCL/Data/PDCL/lignees_count_genes_PDCL.txt", 
  sep = "\t", 
  header = T, 
  as.is = "gene_id", 
  check.names = F
...
plot_common_dereg_percent <- ggplot(data, aes(x = run, y = percent, colour = dereg)) +
  geom_point() +
  geom_hline(yintercept = 0.9, colour = "red", alpha = 0.6) +
  ylim(c(0,1))
