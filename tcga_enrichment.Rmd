...
source("R/pdcl_process_as_1_patient/quickgo.R")
source("R/pdcl_process_as_1_patient/kegg.R")
source("R/pdcl_process_as_1_patient/kegg.R")
source("R/pdcl_process_as_1_patient/reactome.R")

# Constant
result_dir <- "~/PhD/PDCL/Result_tcga_enrichment/"
```
...
sample_table <- read.table("~/PhD/Data/TCGA-GBM/tcga-gbm-sample-table.txt", header = T)
```
...
ensembl_ids <- gsub("\\.\\d+", "", row.names(dds), perl = T)

gconvres <- gconvert(ensembl_ids, organism = "hsapiens", target = "ENSG", filter_na = F)
gene_names <- gconvres$name
names(gene_names) <- gconvres$input
na_gene <- which(is.na(gene_names))
non_unique_gene <- which(!isUnique(gene_names))
keep_id <- c(na_gene, non_unique_gene)
gene_names[keep_id] <- gconvres$input[keep_id]
gene_names <- gene_names[ensembl_ids]
row.names(dds) <- gene_names

deseq2_res <- results(dds, alpha = alpha, contrast = c("condition", "tumor", "solid_tissue_normal"))

saveRDS(deseq2_res, file = paste0(result_dir, "deseq2_tumor_vs_solid_tissue_normal.rds"))
write.csv(as.data.frame(deseq2_res), file = paste0(result_dir, "deseq2/deseq2_tumor_vs_solid_tissue_normal.csv"))
...
```{r compare-lfc}
count_table <- assay(dds)
tumor_index <- which(dds$condition == "tumor")
tumor_mean_expr <- rowMeans(count_table[,tumor_index])
names(tumor_mean_expr) <- colnames(count_table)[tumor_index]
control_mean_expr <- rowMeans(count_table[,-tumor_index])
names(control_mean_expr) <- colnames(count_table)[-tumor_index]
...
```{r save-rds}
saveRDS(all_res, file = paste0(result_dir, "all_res.rds"))
```
...
kegg_entries <- get.kegg.entries(unique(data$pathway_id))
kegg_categories <- get.kegg.categ(kegg_entries)
kegg_categories <- sapply(kegg_categories, function(x){ x[1] })
categories <- kegg_categories[data$pathway_id]
names(categories) <- names(kegg_categories)
categories <- factor(categories, level = KEGG_TOP_CATEGORIES)
...
react_ancestors_list <- get.list.reactome.ancestors( unique(data$pathway_id) )
react_categories <- get.reactome.categories(react_ancestors_list)
categories <- react_categories[data$pathway_id]
categories <- factor(categories, level = REACTOME_TOP_LEVEL_PATHWAYS)
...
all_categ_enriched <- c(kegg_categories, react_categories)
...
save.image("~/PhD/PDCL/tcga_enrichment.RData")
```
