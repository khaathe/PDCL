...
deseq2_res_df$sd_dereg[super_deregulated_genes$gene] <- super_deregulated_genes$dereg
deseq2_res_df$sd_dereg <- as.factor(deseq2_res_df$sd_dereg)
data <- deseq2_res_df %>% filter(sd_dereg != "0")
volcano_sd_genes_lfc <- ggplot(
  data,
  aes(x = log2FoldChange, y = -log10(padj), colour = sd_dereg, alpha = 0.6)
) + 
  geom_point() +
...
```{r num enriched-pathways-per-method}
heatmap.categories(gem_sd_collapsed, 0.1)
```
