...
```{r volcano-plot-deseq2, fig.width=12}
...
```{r plot-sd-genes-lfc, fig.width=12}
...
```{r plot-nes-reactome, fig.width=9, fig.height=8}
data <- all_res[["GSEA"]]$reactome$gbm %>%
  dplyr::filter(padj < threshold) %>%
  dplyr::arrange(padj) %>%
  dplyr::slice(1:25)
...
gsea_reactome_enriched <- all_enrich_collapsed %>%
  dplyr::filter(method == "GSEA", fdr < threshold, db == "reactome") %>%
  dplyr::arrange(fdr) %>%
  dplyr::slice(1:25)
...
gsea_kegg_enriched <- all_enrich_collapsed %>%
  dplyr::filter(method == "GSEA", fdr < threshold, db == "kegg") %>%
  dplyr::arrange(fdr) %>%
  dplyr::slice(1:25)
...
![Gene Ontology Map](Result_pdcl_process_as_1_patient/plot/alpha_0.1/common_pathways_go_chart.png)
